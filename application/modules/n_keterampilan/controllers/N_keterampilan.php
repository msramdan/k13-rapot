...
class N_keterampilan extends CI_Controller {
    ...
    public function import_nilai() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {//jagain barangkali uploadnya selain file excel <span class="wp-smiley wp-emoji wp-emoji-smile" title=":-)">:-)</span>
            ...
            foreach ($data as $d) {
                ...
                $tasm = $d['tasm'];
                $id_guru_mapel = $d['id_guru_mapel'];
                $id_mapel_kd = $d['id_mapel_kd'];
                $id_siswa = $d['id_siswa'];
                $nilai = $d['nilai'];
                // var_dump($tasm); 
                // echo '<br>';
                // var_dump($id_guru_mapel);
                // echo '<br>';
                // var_dump($id_mapel_kd);
                // echo '<br>';
                // var_dump($id_siswa);
                // echo '<br>';
                // var_dump($nilai);
                ...
                $cek_jml = $this->db->query("SELECT * FROM t_nilai_ket 
                                            where tasm='$tasm'
                                            and id_guru_mapel='$id_guru_mapel'
                                            and id_mapel_kd='$id_mapel_kd'
                                            and id_siswa='$id_siswa'");
                $jml = $cek_jml->num_rows();
                if($jml < 1){
                    // jika blm maka tambah
                    $this->db->query("INSERT INTO t_nilai_ket (tasm,id_guru_mapel,id_mapel_kd,id_siswa,nilai)
                    VALUES ('$tasm','$id_guru_mapel','$id_mapel_kd','$id_siswa','$nilai')");
                }else{
                    // jika sudah maka update
                    $this->db->query("UPDATE t_nilai_ket
                                    SET nilai='$nilai'
                                    where tasm='$tasm'
                                    and id_guru_mapel='$id_guru_mapel'
                                    and id_mapel_kd='$id_mapel_kd'
	...
	public function uploadkd($bawa) {
        $pc_bawa = explode("-", $bawa);
        
        $detil_guru = $this->db->query("SELECT 
                                         a.id, a.id_guru, a.id_mapel, b.nama nmmapel, c.nama nmkelas, d.nama nmguru
                                         FROM t_guru_mapel a
                                         INNER JOIN m_mapel b ON a.id_mapel = b.id 
                                         INNER JOIN m_kelas c ON a.id_kelas = c.id
                                         INNER JOIN m_guru d ON a.id_guru = d.id 
	...
	public function import_kd($bawa) {
		
		$pc_bawa = explode("-", $bawa);
		
        $target_file = './upload/temp/';
        move_uploaded_file($_FILES["import_excel"]["tmp_name"], $target_file."import_kd_riduansalim_com.xls");
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            $tmp    = './upload/temp/import_kd_riduansalim_com.xls';
            //Baca dari tmp folder jadi file ga perlu jadi sampah di server :-p
            ...
            $_sheet = $excel->setActiveSheetIndexByName('data_kd');            
			...
			$idmapel = $this->d['idnya'];

            $no = 1;
            for ($b = 2; $b < 500; $b++) {
                $kode = $_sheet->getCell('A'.$b)->getCalculatedValue();
                $isi = $_sheet->getCell('B'.$b)->getCalculatedValue();
				
                if ($kode != "" || $isi != "" ) {
                    $data[] = "('$pc_bawa[0]', '$pc_bawa[1]', '$pc_bawa[2]', '$semester', '$kode', 'K', '$isi' )";
                    $no++;
                }
            }
            ...
            $strq = "INSERT INTO t_mapel_kd(id_guru, id_mapel, tingkat, semester, no_kd, jenis, nama_kd) VALUES ";

            $strq .= implode(",", $data).";";
            
            $this->db->query($strq);

            @unlink('./upload/temp/import_kd_riduansalim_com.xls');
            
            $this->session->set_flashdata('k', '<div class="alert alert-success">'.($no-1).' siswa berhasil diupload</div>');
            redirect('n_keterampilan/index/'. $pc_bawa[3] );
