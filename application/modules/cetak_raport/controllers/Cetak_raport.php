...
class Cetak_raport extends CI_Controller {
    ...
    public function cetak($id_siswa,$tasm) {
        ...
        foreach ($array1 as $k => $v) {
            ...
            if (!empty($array1[$k]['h'])) {
                foreach ($array1[$k]['h'] as $j) {
                    ...
                    $_desk = nilai_pre($kkmx,$pc_nilai_h[0]);
        ...
        $ambil_nk = $this->db->query("SELECT 
                                    c.id idmapel, c.kkm, a.tasm, c.kd_singkat, CONCAT(a.nilai,'//',d.nama_kd) nilai
                                    FROM t_nilai_ket a
                                    INNER JOIN t_guru_mapel b ON a.id_guru_mapel = b.id
                                    INNER JOIN m_mapel c ON b.id_mapel = c.id
                                    INNER JOIN t_mapel_kd d ON a.id_mapel_kd = d.id
        ...
        foreach ($array2 as $k => $v) {
            ...
            foreach ($array2[$k] as $j) {
                ...
                $_desk = nilai_pre($kkmx, $pc_nilai[0]);
        ...
        $kelompok = array("A", "B");
            ...
            if ($this->config->item('is_kemenag') == TRUE) {    
                ...
                foreach ($q_mapel as $i=>$m) {
                    ...
                    $npd = empty($nilai_pengetahuan[$idx]['desk']) ? "-" : "Capaian kompetensi Ananda ".$siswa['nama']." ".$predikatx." dengan predikat ".nilai_pre($kkmx, $npa).". ".$nilai_pengetahuan[$idx]['desk'];
                    ...
                    $nkd = empty($nilai_keterampilan[$idx]['desk']) ? "-" : "Capaian kompetensi Ananda ".$siswa['nama']." ".$predikatx1." dengan predikat ".nilai_pre($kkmx, $nka).". ".$nilai_keterampilan[$idx]['desk'];
