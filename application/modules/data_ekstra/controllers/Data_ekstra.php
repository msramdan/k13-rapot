...
class Data_ekstra extends CI_Controller {
    ...
    public function datatable() {
        ...
        $d_total_row = $this->db->query("SELECT id FROM m_ekstra")->num_rows();
    
        $q_datanya = $this->db->query("SELECT * FROM m_ekstra WHERE nama LIKE '%".$search['value']."%' ORDER BY id DESC LIMIT ".$start.", ".$length."")->result_array();
        $data = array();
        $no = ($start+1);
    ...
    public function simpan() {
        ...
        $d['data'] = "";
        if ($p['_mode'] == "add") {
            $this->db->query("INSERT INTO m_ekstra (nama) VALUES ('".$p['nama']."')");
            $d['status'] = "ok";
