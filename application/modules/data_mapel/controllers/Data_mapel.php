...
class Data_mapel extends CI_Controller {
    ...
    public function simpan() {
        ...
        if ($p['_mode'] == "add") {
            $this->db->query("INSERT INTO m_mapel (kelompok, nama, kd_singkat,kkm) VALUES ('".$p['kelompok']."', '".$p['nama']."', '".$p['kd_singkat']."', '".$p['kkm']."')");

            $d['status'] = "ok";
            $d['data'] = "Data berhasil disimpan";
        } else if ($p['_mode'] == "edit") {
            $this->db->query("UPDATE m_mapel SET kelompok = '".$p['kelompok']."', kd_singkat = '".$p['kd_singkat']."', nama = '".$p['nama']."',kkm = '".$p['kkm']."' WHERE id = '".$p['_id']."'");
