    ...
    public function import_nilai()
    {
        ...
        if ($file[$length - 1] == 'xlsx' || $file[$length - 1] == 'xls') {
            ...
            foreach ($data as $d) {
                ...
                // echo '<br>';
                // var_dump($nilai);
                $cek_jml = $this->db->query("SELECT * FROM t_nilai 
                                            where tasm='$tasm'
                                            and jenis='$jenis'
                ...
                if($jml < 1){
                    ...
                    $this->db->query("INSERT INTO t_nilai (tasm,jenis,id_guru_mapel,id_mapel_kd,id_siswa,nilai)
                    VALUES ('$tasm','$jenis','$id_guru_mapel','$id_mapel_kd','$id_siswa','$nilai')");
                }else{
                    // jika sudah maka update
                    $this->db->query("UPDATE t_nilai
                                    SET nilai='$nilai'
                                    where tasm='$tasm'
                                    and jenis='$jenis'
                                    and id_guru_mapel='$id_guru_mapel'
    ...
    public function import_kd($bawa)
    {
        ...
        if ($file[$length - 1] == 'xlsx' || $file[$length - 1] == 'xls') {
            ...
            for ($b = 2; $b < 500; $b++) {
                ...
                if ($kode != "" || $isi != "") {
                    $data[] = "('$pc_bawa[0]', '$pc_bawa[1]', '$pc_bawa[2]', '$semester', '$kode', 'P', '$isi' )";
                    $no++;
                }
            }
            ...
            redirect('n_pengetahuan/index/' . $pc_bawa[3]);
        } else {
            exit('Buka File Excel...'); //pesan error tipe file tidak tepat
        }
        redirect('n_pengetahuan/index/' . $pc_bawa[3]);
